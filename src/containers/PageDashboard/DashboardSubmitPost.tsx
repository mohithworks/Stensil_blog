...
const DashboardSubmitPost = () => {
  ...
  const uploadFi = async (postTitle:any) => {
    ...
    for(var i = 0 ; i <= 1; i++) {
      ...
      const imagepath = 'public/'+postTitle+'/featuredImg/'+ imagesName[i];
  ...
  const createPost = async (e: any) => {
    ...
    if(title == "" || category == undefined || category == "-1" || fihighRes == undefined || filowRes == undefined) {
      ...
      const options = {
        ...
        params: {fileId: docs, accessToken: oauthToken, title: postTitle },
      }
        ...
        if(res.posttitle == postTitle){
          ...
          uploadFi(postTitle).then(() => { 
            ...
            axios.request(options).then(async (response) => {
                ...
                setbtnLoading(true);
                authUser = supabaseClient.auth.user();
                ...
                const postUrl = '/'+authorusername+'/posts/'+postTitle;
                const { data, error } = await supabaseClient.from('posts').insert(
                  [
                    { posttitle: postTitle, title: title, category: category, post: response.data, featured_imghd: fihd, featured_imgsd: fisd, postedby: authorusername, docsid: docs, href: postUrl },
                  ],
                  { upsert: true }
                );
                if(error) throw alertMsg(error.message, "error");
